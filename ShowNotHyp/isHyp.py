...
def hasTorusBoundary(mfld):
  ...
  for v in m.getVertices():
    if not v.getLink() == NVertex.TORUS:
      return False
  return True

def irreducible(regina_mfld):
  M = Triangulation(regina_mfld)
  M.finiteToIdeal()
...
def isHyp(regina_manifold):
  # m = Triangulation3(regina_manifold)
  m=regina_manifold
  m.intelligentSimplify()
